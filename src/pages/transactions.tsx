  ...
  async function handleSubmit(event: TargetedEvent<HTMLFormElement, Event>): Promise<void> {
    ...
    try {
      const payload = {
        ...
        body: JSON.stringify({
          postedOn: postedOnInput.value,
          transactionOn: transactionOnInput.value,
        ...
        <tbody>
          {transactions.value.map((transaction) => (
            <tr key={transaction.id}>
              ...
              <td class="py-2 px-4 border">{transaction.postedOn}</td>
              <td class="py-2 px-4 border">{transaction.transactionOn}</td>
